...
<c:url var="ajaxData" value="/inventory/brands/ajax-data-brands"/>
<c:url var="jsFile" value="/js/pagescroll.js"/>
<c:if test="${actionview=='detail'}">
	<c:url var="ajaxData" value="/inventory/catalogs/ajax-data-catalogs-inbrands/${id}"/>
...
<c:set var="ajaxData" value="/inventory/brands/ajax-data-brands"/>
<c:set var="jsFile" value="/js/pagescroll.js"/>
<c:if test="${actionview=='detail'}">
	<c:set var="ajaxData" value="/inventory/catalogs/ajax-data-catalogs-inbrands/${id}"/>
...
function changeStatusAll(stat) {
	$('#dataList').attr('action',"${baseURL}inventory/brands/delete?status="+stat);
	$('#dataList').submit();
}

function changeStatus1(id,stat) {
	$('#frList').attr('action',"${baseURL}inventory/brands/status-change");
	...
	$(document).ready(function() {
		...
		$('#btnHapusSelected').on('click',function(e){
			e.preventDefault();
			$(this).blur();
			if ($('input[name="idBrand[]"]:checked').length>0) {
				$.colorbox({
					inline : true,
					height:161,
					href : "#popup"
		...
		$('#tableValue').bind('click', function(event) {
			...
			if (elementHere.getAttribute('class').indexOf("btnHapus1")!=-1) {
				...
				if (elementHere.getAttribute('candel')=='true') {
					$.colorbox({
						inline : true,
						height:161,
						href : "#askDelete1"
		...
		$('.btnTahan1').on('click',function(e){
			e.preventDefault();
			changeStatus1($(this).attr('id'),0);
		 });
		
		$('.btnAktif1').on('click',function(e){
			e.preventDefault();
			changeStatus1($(this).attr('id'),1);
		 });
