		...
		<div class="grid-full">
			<div class="grid-content">
				...
				<div class="print-layout">
					<div class="print-head">
						...
						<div class="print-info">
							...
							<p><fmt:formatDate pattern="dd/MM/yyyy" value="${trxDate}" /></p>
						</div>
						<div class="clear"></div>
					</div>
				...
				<div class="content-header">
					...
					<h1>Payment Issued ${transactionId}</h1>
...
<%-- 								<c:url var="studentProfile" value="/academic/students/${oForm.studentId}/details" /> --%>
								<a href="javascript:void(0)">${oForm.vendorName}</a>
							<%-- </c:when>
							<c:otherwise>
								${oForm.customerName}
							</c:otherwise>
				...
				<table class="table table-striped">
					...
					<tbody>
						<c:if test="${!empty oForm.paidAmounts}">
							<c:forEach items="${oForm.paidAmounts}" var="paidAmount" varStatus="status">
								<tr>
									<td>
										<c:url var="billUrl" value="/finance/bills" />
										<h4><a href="${billUrl}/${oForm.billIds[status.index]}/view">${oForm.billTitles[status.index]}</a></h4>
										<span>${oForm.companyNames[status.index]}</span> &mdash;
										<span>${oForm.accountNames[status.index]}</span>
									</td>
									<td>
										<span class="amount">${oForm.billAmounts[status.index]}</span>
									</td>
									<td>
					...
					<tfoot>
						<tr>
							<td></td>
							<td><h4 class="align-right">Total (Rp.)</h4></td>
